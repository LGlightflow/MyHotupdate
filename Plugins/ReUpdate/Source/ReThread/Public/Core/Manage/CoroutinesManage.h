...
struct RETHREAD_API FCoroutinesManage :FTreadTemplateBase<ICoroutinesContainer, FCoroutinesHandle>
{
public:
	FCoroutinesManage();
...
public:
	...
	{
		*this << InTotalTime << FOnGoingDelegate::CreateLambda(InMethod, Vars...);
	}
	...
	{
		*this << InTotalTime << FOnGoingDelegate::CreateRaw(TargetClass, InMethod, Vars...);
	}
	...
	{
		*this << InTotalTime << FOnGoingDelegate::CreateUObject(TargetClass, InMethod, Vars...);
	}
	...
	{
		*this << InTotalTime << FOnGoingDelegate::CreateSP(TargetClass, InMethod, Vars...);
	}

	template<class UserClass, typename... VarTypes>
	void BindOnGoingUFunction(float InTotalTime, UserClass* TargetClass, const FName& InMethod, VarTypes... Vars)
	{
		*this << InTotalTime << FOnGoingDelegate::CreateUFunction(TargetClass, InMethod, Vars...);
	}
};