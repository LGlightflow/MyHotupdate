...
namespace ReVersionControl
{
	...
	void Save(const FVersionInfor& Infor, FString& Json)
	{
		...
		for (const auto& Tmp : Infor.Content)
		{
			...
			JsonWriter->WriteValue(TEXT("bMainVersion"), Tmp.bMajorVersion);
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			{		
				for (const auto& Tmp : *JsonArrays)
				{
					...
					if (!TmpJsonObject->TryGetBoolField(TEXT("bMainVersion"), RemoteDataDescribe.bMajorVersion))
					{
						bReadSuccess = false;
					}
	...
	void Save(const FRemoteDataDescribe& Infor, FString& Json)
	{
		...
		JsonWriter->WriteObjectStart();
		{
			...
			JsonWriter->WriteValue(TEXT("bMainVersion"), Infor.bMajorVersion);
	...
	bool Read(const FString& Json, FRemoteDataDescribe& Infor)
	{
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			if (const TSharedPtr<FJsonObject> TmpJsonObject = JsonParsed->AsObject())
			{
				...
				Infor.bMajorVersion = TmpJsonObject->GetBoolField(TEXT("bMainVersion"));
...
bool FReVersionBase::IsVaild() const
{
	return !Crc.IsEmpty();
}
